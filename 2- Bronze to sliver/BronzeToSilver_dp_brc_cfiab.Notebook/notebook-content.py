...
#    Exemple filename : PSID099.CMN.BRC.ListeDetail.Entreprise-CFIAB-Stock-Entrees-Sorties-Taches-HEBDO.20251102.csv
...
df_silver = df.select(
    ...
    trim(col("Anciennete_Tache")).alias("Anciennete_Tache"),
    ...
    trim(col("Libelle_information_complementaire_nombre_3")).alias("Libelle_information_complementaire_nombre_3"),
    col("Valeur_information_complementaire_nombre_3").cast(IntegerType()).alias("Valeur_information_complementaire_nombre_3"),
    ...
    trim(col("Identifiant_Public_Type_Objet_CTX2")).alias("Identifiant_Public_Type_Objet_CTX2"),
...
spark.sql(f"DELETE FROM dp_brc_cfiab WHERE Numero_Semaine IN ({semaines_str})")

# Insérer les nouvelles données en mode append
df_silver.write \
    .format("delta") \
    .mode("append") \
    .insertInto("dp_brc_cfiab")

print(f"Semaines traitées : {semaines}")
print(f"Nombre de lignes total : {spark.table('dp_brc_cfiab').count()}")
